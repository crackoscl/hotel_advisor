...
class Principal(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        comentarios = Comentarios.objects.annotate(
            rating=ExpressionWrapper(
                F("calificacion_comentario") * (100 / 5), output_field=IntegerField()
            )
        )[:4]

        list_hoteles = Hoteles.objects.annotate(
            rating=ExpressionWrapper(
...
class Review(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        promedio = Comentarios.objects.filter(id_hotel=kwargs["pk"]).aggregate(
            Avg("calificacion_comentario"), Count("nombre_usuario")
        )
...
def filter(request, pk):
    ...
    if pk == "nombre":
        ...
        if pk.isdigit():
            comentarios = Comentarios.objects.annotate(
                rating=ExpressionWrapper(
                    F("calificacion_comentario") * (100 / 5),
            ...
            ).filter(calificacion__range=[pk, pk + 0.9])
        else:
            return redirect("app:principal")
    paginator = Paginator(list_hoteles, 3)
    pagina = request.GET.get("page") or 1
...
def search(request):
    ...
    hoteles = Hoteles.objects.annotate(
        rating=ExpressionWrapper(
            F("calificacion") * (100 / 5), output_field=IntegerField()
        )
    ).filter(Q(nombre__icontains=q) | Q(descripcion__icontains=q))
    return render(
        request, "app/index.html", {"hoteles": hoteles,
                                    "comentarios": comentarios}
    )
